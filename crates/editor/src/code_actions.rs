...
impl Editor {
    ...
    pub fn code_actions_menu_origin(&self) -> Option<ContextMenuOrigin> {
        let context_menu = self.context_menu.borrow();
        let Some(menu @ CodeContextMenu::CodeActions(_)) = context_menu.as_ref() else {
            return None;
        };
        Some(menu.origin())
    }
...
impl Editor {
    pub(super) fn confirm_code_action(
        &mut self,
